...
const MakeARentalForm = (props: Props) => {
    ...
    useEffect(() => {
      ...
      if(res1.status === 'success') {
        const res2 = await fetchData(`${DOMAIN_NAME}/reservations/fetchallofcar/${params.carid}?time=future`, (arg:ApiResponse)=>{setFailData(arg)}, (arg:boolean)=>{setFail(arg)}, (arg:boolean)=>{setError(arg)})
        setData2(res2.data.reservations.length);
        if(res2.status === 'success') {
          const res3 = await fetchData(`${DOMAIN_NAME}/rentals/car/${params.carid}/last`, (arg:ApiResponse)=>{setFailData(arg)}, (arg:boolean)=>{setFail(arg)}, (arg:boolean)=>{setError(arg)})
          setData3(res3);
          if(res3.status === 'success' && res3.data !== null) {
            const res4 = await fetchData(`${DOMAIN_NAME}/users/fetchone/${res3.data.userID}`, (arg:ApiResponse)=>{setFailData(arg)}, (arg:boolean)=>{setFail(arg)}, (arg:boolean)=>{setError(arg)})
            setData4(res4);
          }
        }
      }
