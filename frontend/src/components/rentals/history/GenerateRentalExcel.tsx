  ...
  export default function GenerateRentalExcel(props: GenerateRentalExcelProps) {
    ...
    const [pageState, setPageState] = useState<generateExcelPageState>(generateExcelPageState.initial);
      ...
      const getData = async () => {
        ...
        const res1 = await fetchData(`${BACKEND_URL}/rentals?filters=${props.filters}&pagenumber=1&sortby=${props.sortBy}&sortorder=${props.sortOrder}&pagesize=999999999`, (arg:ApiResponse)=>{setFailData(arg)}, (arg:boolean)=>{setFail(arg)}, (arg:boolean)=>{setError(arg)})
        if(res1.status==='success') {
          ...
          if(res2.status==='success') {
            ...
            if(res3.status==='success') {
              const res4 = await fetchData(`${BACKEND_URL}/places?filters=%7B%7D&pagenumber=1&sortby=createdAt&sortorder=ASC&pagesize=999999999`, (arg:ApiResponse)=>{setFailData(arg)}, (arg:boolean)=>{setFail(arg)}, (arg:boolean)=>{setError(arg)})
