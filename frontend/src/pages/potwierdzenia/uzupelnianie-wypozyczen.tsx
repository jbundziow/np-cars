    ...
    useEffect(() => {
      const getData = async () => {  
        ...
        if(res1.status === 'success') {
          const res2 = await fetchData(`${BACKEND_URL}/rentals/gaps?carid=all&excludeonerental=false`, (arg:ApiResponse)=>{setFailData(arg)}, (arg:boolean)=>{setFail(arg)}, (arg:boolean)=>{setError(arg)})
          setData2(res2);
        }
        setLoading(false)
      }
