...
const RefuelingArchive = (props: Props) => {
    ...
    useEffect(() => {
      const getData = async () => {
        ...
        const res1 = await fetchData(`${BACKEND_URL}/refuelings?filters=${filters}&pagenumber=${currentPage}&sortby=${sortBy}&sortorder=${sortOrder}&pagesize=${pageSize}`, (arg:ApiResponse)=>{setFailData(arg)}, (arg:boolean)=>{setFail(arg)}, (arg:boolean)=>{setError(arg)})
        setData1(res1);
