...
const MakeARentalAdminForm = (props: Props) => {
    ...
    useEffect(() => {
      ...
      const res1 = await fetchData(`${BACKEND_URL}/places?filters=%7B%22projectCode%22%3A%22%22%2C%22placeName%22%3A%22%22%2C%22projectName%22%3A%22%22%2C%22status%22%3A%22active%22%7D&pagenumber=1&sortby=createdAt&sortorder=ASC&pagesize=99999999`, (arg:ApiResponse)=>{setFailData(arg)}, (arg:boolean)=>{setFail(arg)}, (arg:boolean)=>{setError(arg)})
      setData1(res1);
      if(res1.status==='success') {
        const res2 = await fetchData(`${BACKEND_URL}/cars/?basicdata=true&showbanned=false`, (arg:ApiResponse)=>{setFailData(arg)}, (arg:boolean)=>{setFail(arg)}, (arg:boolean)=>{setError(arg)})
        setData2(res2);
        if(res1.status==='success') {
          const res3 = await fetchData(`${BACKEND_URL}/users/?showbanned=false`, (arg:ApiResponse)=>{setFailData(arg)}, (arg:boolean)=>{setFail(arg)}, (arg:boolean)=>{setError(arg)})
          setData3(res3);
        }
