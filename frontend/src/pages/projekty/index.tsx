...
const PlaceMainpage = (props: Props) => {
  ...
  const currentYear = new Date().getFullYear();
    ...
    useEffect(() => {
      const getData = async () => {   
        const res3 = await fetchData(`${BACKEND_URL}/stats/places/${params.placeid}/distance?year=${filterValue}`, (arg:ApiResponse)=>{setFailData(arg)}, (arg:boolean)=>{setFail(arg)}, (arg:boolean)=>{setError(arg)})
        setData3(res3)
      }
      getData()
    ...
    useEffect(() => {
      ...
      const res1 = await fetchData(`${BACKEND_URL}/places/${params.placeid}?showbanned=true`, (arg:ApiResponse)=>{setFailData(arg)}, (arg:boolean)=>{setFail(arg)}, (arg:boolean)=>{setError(arg)})
      setData1(res1);
      
      const res2 = await fetchData(`${BACKEND_URL}/stats/places/${params.placeid}/total`, (arg:ApiResponse)=>{setFailData(arg)}, (arg:boolean)=>{setFail(arg)}, (arg:boolean)=>{setError(arg)})
      setData2(res2)
      
      const res4 = await fetchData(`${BACKEND_URL}/stats/places/${params.placeid}/total/year/${currentYear}`, (arg:ApiResponse)=>{setFailData(arg)}, (arg:boolean)=>{setFail(arg)}, (arg:boolean)=>{setError(arg)})
      setData4(res4)
      
      const res5 = await fetchData(`${BACKEND_URL}/stats/places/${params.placeid}/distance/users`, (arg:ApiResponse)=>{setFailData(arg)}, (arg:boolean)=>{setFail(arg)}, (arg:boolean)=>{setError(arg)})
      setData5(res5)
      
      const res6 = await fetchData(`${BACKEND_URL}/stats/places/${params.placeid}/distance/bycartypes?year=${currentYear}`, (arg:ApiResponse)=>{setFailData(arg)}, (arg:boolean)=>{setFail(arg)}, (arg:boolean)=>{setError(arg)})
      setData6(res6)
      
      const res7 = await fetchData(`${BACKEND_URL}/stats/places/${params.placeid}/favourite/car?year=${currentYear}`, (arg:ApiResponse)=>{setFailData(arg)}, (arg:boolean)=>{setFail(arg)}, (arg:boolean)=>{setError(arg)})
      setData7(res7)
      
      const res8 = await fetchData(`${BACKEND_URL}/stats/places/${params.placeid}/favourite/user?year=${currentYear}`, (arg:ApiResponse)=>{setFailData(arg)}, (arg:boolean)=>{setFail(arg)}, (arg:boolean)=>{setError(arg)})
      setData8(res8)

      const res9 = await fetchData(`${BACKEND_URL}/stats/places/${params.placeid}/distance/cars`, (arg:ApiResponse)=>{setFailData(arg)}, (arg:boolean)=>{setFail(arg)}, (arg:boolean)=>{setError(arg)})
      setData9(res9)
