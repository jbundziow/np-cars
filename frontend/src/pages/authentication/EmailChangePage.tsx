...
const EmailChangePage = () => {
  ...
  useEffect(() => {
    ...
    if(query.get('userfullname')) {setUserFullname(query.get('userfullname'))}
    if(query.get('old_email')) {setOldEmail(query.get('old_email'))}
    if(query.get('new_email')) {setNewEmail(query.get('new_email'))}
    }, [query]);
  ...
  const submitHandler = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
      ...
      try {
        ...
        const response = await fetch(`${BACKEND_URL}/auth/email_change`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json; charset=utf-8',
          },
          credentials: 'include',
          body: JSON.stringify({token, userid, new_email: newEmail}),
        });
        if (response.ok) {
       ...
       <div className="max-w-[1400px] rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark my-20 mx-3">
        <div className="flex flex-wrap items-center">
          ...
          <div className="w-full border-stroke dark:border-strokedark xl:w-3/5 xl:border-l-2">
            <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
              ...
              <div className="mb-5">
                ...
                <div className="mt-6 text-center">
                  <p>
                    Nie chcesz zmieniać adresu email?{' '}
            ...
            <>
            <OperationResult status={'success'} title={'Pomyślnie dokonano zmiany adresu email 👍'} description={'Zmiany zostały zapisane.'} showButton={false}/>
            <div className="mt-6 text-center">
              <p>
            ...
            <OperationResult status={'error'} title={'Wystąpił błąd podczas zmiany adresu email 😭'} description={'Spróbuj ponownie później lub skontaktuj się z administratorem.'} showButton={true} buttonText={'Spróbuj ponownie'} onClick={()=> setPageState(FormPageStatus.FillingTheForm)}/>
            :
            pageState === FormPageStatus.FailOnSendingForm ?
            <OperationResult status={'warning'} title={'Wystąpiły błędy podczas zmiany adresu email 🤯'} warnings={warnings} showButton={true} buttonText={'Spróbuj ponownie'} onClick={()=> setPageState(FormPageStatus.FillingTheForm)}/>
            :
            <></>
            }
