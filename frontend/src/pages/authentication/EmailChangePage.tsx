...
const EmailChangePage = () => {
  ...
  useEffect(() => {
    ...
    if (query.get('userfullname')) {
      setUserFullname(query.get('userfullname'));
    }
    if (query.get('old_email')) {
      setOldEmail(query.get('old_email'));
    }
    if (query.get('new_email')) {
      setNewEmail(query.get('new_email'));
    }
  }, [query]);
            ...
            <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
              {pageState === FormPageStatus.FillingTheForm ? (
                ...
                <>
                  <OperationResult
                    status={'success'}
                    title={'Pomyślnie dokonano zmiany adresu email 👍'}
                ...
                <OperationResult
                  status={'error'}
                  title={'Wystąpił błąd podczas zmiany adresu email 😭'}
                  description={
                ...
                <OperationResult
                  status={'warning'}
                  title={'Wystąpiły błędy podczas zmiany adresu email 🤯'}
                  warnings={warnings}
                  showButton={true}
                  buttonText={'Spróbuj ponownie'}
                  onClick={() => setPageState(FormPageStatus.FillingTheForm)}
