...
const MyReservations = (props: Props) => {
    ...
    useEffect(() => {
      const getData = async () => {   
      const res1 = await fetchData(`${BACKEND_URL}/reservations/users/${auth.userID}?time=future`, (arg:ApiResponse)=>{setFailData(arg)}, (arg:boolean)=>{setFail(arg)}, (arg:boolean)=>{setError(arg)})
      setData1(res1);
